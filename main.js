...
const ordenarAlfabeticamente = () => {
    stock.sort((a) => a.nombre)
    mostrarStock()
}

const mostrarStock = () => {
    const listaStock = stock.map(producto => {
        return '-' + producto.nombre
    })
    alert('Lista de Productos para agregar al Stock del inventario:' + '\n\n' + listaStock.join('\n'))
    agregarArticulos(listaStock)
}


const agregarArticulos = (listaStock) => {
    let cantidad = 0
    let totalProductoAlta = 0

    do {
        producto = prompt('¿Que producto deseas dar de alta:' + '\n\n' + listaStock.join('\n'))
